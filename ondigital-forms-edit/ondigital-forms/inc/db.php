...
function odf_create_table(): void {
    ...
    dbDelta( $sql );
    update_option( 'odf_db_version', ODF_DB_VERSION );
}

// Run migration when plugin updates (dbDelta adds missing columns safely)
add_action( 'admin_init', 'odf_maybe_upgrade_db' );
function odf_maybe_upgrade_db(): void {
    if ( get_option( 'odf_db_version' ) !== ODF_DB_VERSION ) {
        odf_create_table();
    }
}
...
function odf_default_about_page_options(): array {
    return array(
        'form_title_az'  => 'Əlaqə saxlayın',
        'form_title_en'  => 'Get in Touch',
        'btn_text_az'    => 'Göndər',
        'btn_text_en'    => 'Send',
        'success_az'     => 'Mesajınız göndərildi! Tezliklə sizinlə əlaqə saxlayacağıq.',
        'success_en'     => 'Your message has been sent! We will contact you shortly.',
